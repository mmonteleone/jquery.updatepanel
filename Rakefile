...
task :build => [:clean] do
  ...
  # copy src
  cp 'jquery.updatepanel.js', 'dist/jquery.updatepanel.js'
  ...
  # minify src
  source = File.read('dist/jquery.updatepanel.js')
  minified = Packr.pack(source, :shrink_vars => true, :base62 => false)
  header = /\/\*.*?\*\//m.match(source)

  # inject header
  File.open('dist/jquery.updatepanel.min.js', 'w') do |combined|
...
task :release => [:build] do
  root = pwd+'/dist'
  zip_archive = pwd+'/dist/jquery.updatepanel.zip'
...
task :test => [:build] do  
  begin
    # mac
    sh("open spec/jquery.updatepanel.specs.html")
  rescue
    # windows
    sh("start spec/jquery.updatepanel.specs.html")
  end
end
